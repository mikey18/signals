...
class Premium_Trade:
        ...
        if trade_status == "loss":
            if self.current_step < 3:
                ...
                if self.current_phase < 11:
                    ...
                    self.initial_lot_size = await self.calculate_initial_lot_size(new_balance)
            ...
            if self.current_phase > 0:
                new_balance = mt5.account_info().balance
                if new_balance > self.phase_initial_balances[(self.current_phase + 1) - 1]:
                    self.current_phase -= 1
                    self.current_step = 0
                else:
                    self.current_step = 0
    ...
    async def money_management(self):
        ...
        while True:
            phases = {
                phase: [(phase * self.initial_lot_size, 250, 750), 
                        (phase * self.initial_lot_size, 250, 750), 
                        (phase * self.initial_lot_size, 500, 1500), 
                        (phase * self.initial_lot_size, 1000, 3000)]
